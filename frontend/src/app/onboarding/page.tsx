    ...
    <div style={{ flex: flex || 1, position: "relative" }}>
        ...
        <select value={value} onChange={(e) => onChange(e.target.value)}
            ...
            style={{
                ...
                WebkitTapHighlightColor: "transparent", colorScheme: "dark", cursor: "pointer"
            }}>
            {options.map((opt: any) => (
                <option key={opt.value || opt} value={opt.value || opt}>{opt.label || opt}</option>
            ))}
        </select>
        <div style={{ position: "absolute", right: 10, bottom: 10, pointerEvents: "none", opacity: 0.3, fontSize: 8 }}>▼</div>
    </div>
);
...
function AstroFlow({ step, setStep, onBack }: { step: number, setStep: React.Dispatch<React.SetStateAction<number>>, onBack: () => void }) {
    ...
    const steps = [
        ...
        { id: "date", title: "Дата рождения", hint: "Точная дата — основа вашей карты", icon: "✦", iconColor: "var(--gold)", description: "Дата формирует ядро архетипической карты" },
        { id: "time", title: "Время рождения", hint: "Если не знаете — укажите 12:00", icon: "◉", iconColor: "var(--violet-l)", description: "Точное время раскрывает Асцендент" },
        { id: "place", title: "Место рождения", placeholder: "Москва, Россия", hint: "Введите город и страну", icon: "◈", iconColor: "var(--emerald)", description: "Координаты влияют на точность" },
        { id: "confirm", title: "Подтверждение", hint: "Проверьте координаты", icon: "✓", iconColor: "var(--emerald)", description: "Убедитесь в правильности данных" },
    ];
    ...
    const handleNext = async () => {
        ...
        if (currentStep.id === "place") {
            ...
            try {
                ...
                if (msg.toLowerCase().includes("user not found")) { reset(); router.push("/"); return; }
        ...
        try {
            await calcAPI.calculate({
                ...
                birth_place: form.birth_place, user_id: userId!, gender: form.gender
            });
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full">
            <AnimatePresence mode="wait">
                <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
                    ...
                    <div className="flex items-center gap-3 mb-4">
                        <div style={{ width: 38, height: 38, borderRadius: 12, background: "rgba(255,255,255,0.05)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, color: currentStep.iconColor, flexShrink: 0 }}>{currentStep.icon}</div>
                        ...
                        {currentStep.id === "gender" ? (
                            <div className="grid grid-cols-2 gap-3">
                                {GENDER_OPTIONS.map(opt => (
                                    <button key={opt.value} onClick={() => setForm(f => ({ ...f, gender: opt.value }))}
                                        className={`p-3 rounded-xl border transition-all flex flex-col items-center gap-2 ${form.gender === opt.value ? "bg-violet-500/10 border-violet-500/40 text-white" : "bg-white/5 border-white/10 text-white/40 hover:bg-white/10"}`}>
                            ...
                            <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-2xl p-4 text-center">
                                <div className="flex flex-col items-center mb-4">
                                    <div className="w-10 h-10 rounded-full bg-emerald-500/20 flex items-center justify-center mb-2"><span className="text-emerald-400">📍</span></div>
                                ...
                                <div className="grid grid-cols-3 gap-2 py-3 border-t border-emerald-500/10 text-[10px] text-white/40 font-mono">
                                    <div className="flex flex-col"><span className="mb-0.5">ШИРОТА</span><span className="text-white/80 font-bold">{geoResult?.lat != null ? geoResult.lat.toFixed(4) : "—"}°</span></div>
                                    <div className="flex flex-col"><span className="mb-0.5">ДОЛГОТА</span><span className="text-white/80 font-bold">{geoResult?.lon != null ? geoResult.lon.toFixed(4) : "—"}°</span></div>
                                    <div className="flex flex-col"><span className="mb-0.5">ЗОНА</span><span className="text-white/80 font-bold truncate">{geoResult?.tz_name?.split('/').pop() || "—"}</span></div>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <button onClick={handleNext}
                ...
                style={{ background: (currentStep.id === "place" && !form.birth_place) ? "rgba(255,255,255,0.05)" : "linear-gradient(135deg, var(--violet), #6366f1)", color: "#fff" }}>
...
function AIFlow({ onBack }: { onBack: () => void }) {
    ...
    const sendChat = async (hist: { role: string, content: string }[]) => {
        setLoading(true); setMessages(hist);
        try {
        ...
        } catch { setMessages([...hist, { role: "assistant", content: "Ошибка связи с ядром." }]); }
        finally { setLoading(false); }
    };
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            style={{ position: "fixed", inset: 0, display: "flex", flexDirection: "column", background: "#060818", zIndex: 10 }}>
            <div style={{ padding: "12px 16px 8px", borderBottom: "1px solid rgba(255,255,255,0.05)", flexShrink: 0 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <button onClick={onBack} disabled={calcLoading} style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", background: "none", border: "none", cursor: "pointer", padding: "4px 0" }}>← Назад</button>
            ...
            <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: "12px 16px", display: "flex", flexDirection: "column", gap: 10, scrollbarWidth: "none" }}>
                ...
                {loading && <div style={{ display: "flex", justifyContent: "flex-start" }}><div style={{ padding: "10px 14px", borderRadius: "18px 18px 18px 4px", background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.4)", fontSize: 12 }} className="animate-pulse">Печатаю...</div></div>}
            </div>
            ...
            <div style={{ flexShrink: 0, padding: "10px 16px 20px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", flexDirection: "column", gap: 8 }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center", position: "relative" }}>
                    <div style={{ flex: 1, position: "relative" }}>
                        <textarea ref={textareaRef} rows={1} value={isTranscribing ? "Транскрибирую..." : input}
                            onChange={handleTextChange}
                            onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
                            placeholder="Ваш ответ..." disabled={loading || calcLoading || isTranscribing}
                            style={{ width: "100%", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 16, padding: "14px 48px 14px 16px", fontSize: 14, color: "#fff", outline: "none", fontFamily: "inherit", boxSizing: "border-box", resize: "none", overflowY: "auto", maxHeight: "50vh", lineHeight: "1.5", display: "block" }}
                            className="placeholder:text-white/20 focus:border-amber-500/50" />
                        <button onClick={handleSend} disabled={!input.trim() || loading || calcLoading || isTranscribing}
                            style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", width: 32, height: 32, borderRadius: 10, border: "none", cursor: "pointer", background: "rgba(245,158,11,0.2)", color: "#FCD34D", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, opacity: (!input.trim() || loading) ? 0.3 : 1 }}>↑</button>
                    </div>
                    <button onPointerDown={startRecording} onPointerUp={stopRecording} onPointerLeave={stopRecording}
                        style={{ flexShrink: 0, width: 52, height: 52, borderRadius: 16, cursor: "pointer", background: isRecording ? "#EF4444" : "rgba(255,255,255,0.06)", border: isRecording ? "none" : "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", justifyContent: "center", transform: isRecording ? "scale(0.95)" : "scale(1)", transition: "all 0.15s" }}>
                        {isRecording ? "🔴" : <MicIcon className="w-5 h-5 text-amber-500/60" />}
                    </button>
                </div>

                {ready ? (
                    <motion.button onClick={handleCalculate} disabled={calcLoading}
                        style={{ width: "100%", padding: "15px", borderRadius: 18, border: "none", cursor: "pointer", background: "linear-gradient(135deg, #F59E0B, #F97316)", color: "#fff", fontWeight: 900, fontSize: 15, letterSpacing: "0.02em" }}>
...
export default function OnboardingPage() {
    ...
    const [path, setPath] = useState<"astro" | "ai" | null>("astro");
    ...
    useEffect(() => {
        const checkDebug = async () => {
            ...
            if (isDebug) {
                ...
                } catch (e) { console.error("Debug login failed", e); }
            }
        };
        checkDebug();
    }, [setUser]);
    ...
    const particles = useMemo(() =>
        Array.from({ length: 24 }, (_, i) => ({
            ...
            opacity: 0.04 + (i % 5) * 0.04, duration: 2.5 + (i % 4),
            delay: (i % 6) * 0.4, size: i % 3 === 0 ? 2 : 1,
        })), []);

    return (
        <div className="min-h-screen flex flex-col items-center justify-center px-4 relative overflow-hidden bg-[#060818]">
            <div className="absolute inset-0 pointer-events-none">
                <div style={{ position: "absolute", top: "10%", left: "15%", width: 280, height: 280, background: "radial-gradient(circle, rgba(139,92,246,0.12) 0%, transparent 70%)", borderRadius: "50%", filter: "blur(32px)" }} />
                <div style={{ position: "absolute", bottom: "15%", right: "10%", width: 220, height: 220, background: "radial-gradient(circle, rgba(245,158,11,0.10) 0%, transparent 70%)", borderRadius: "50%", filter: "blur(28px)" }} />
            </div>
